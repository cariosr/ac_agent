...
def scape_map(self, x, y, arena, bomb_xys, others, bomb_map):

    scape = np.zeros(4, dtype=int)
...
def scape_map_others(self, arena, bomb_xys, others, bomb_map):

    scape_others = np.zeros(12, dtype=int)

    indx = 0
    if self.flag_others_in_target:
        for ro in others:
            xo,yo = ro
            scape_others[indx:indx+4] = scape_map(self, xo, yo, arena, bomb_xys, others, bomb_map)
            indx += 4

    return scape_others

def get_free_cells(self, x, y, xa, ya, arena, bomb_map, others,bomb_xys):
    queue = deque([(xa, ya)])
    visited = {}
    visited[(x, y)] = 0
    visited[(xa, ya)] = 1
...
def mappping(self):
    ...
    for xb, yb, t in bombs:
        ...
        if tp != 0 :
            if tp == 1:
                dead_zone[xb, yb] = min(dead_zone[xb, yb], tp)
            else:
                for (i, j) in [(xb + h, yb) for h in range(-tp+1, tp)] + [(xb, yb + h) for h in range(-tp+1, tp)]:
                    if (0 < i < dead_zone.shape[0]) and (0 < j < dead_zone.shape[1]):
                        dead_zone[i, j] = min(dead_zone[i, j], tp)

    #Valid state
    state[:4] = valid_action(self, x, y, arena, bombs, bomb_xys, others, dead_zone)
    # 1 bit  for flag bomb
    state[4] = bombs_left

    # Building the target state..
    # List of 'targets' the agent should head towards

    self.flag_others_in_target = 0
    # if (bit_action != 'WAIT'):
    #     idx_bit = self.map_actions[bit_action]
    #     state[5 + idx_bit] = 1
    for ro in others:
        xo, yo = ro
        if (np.absolute(xo - x) + np.absolute(yo - y)) < 5:
            self.flag_others_in_target = 1
        # self.flag_others_in_target = 1
   # else:
    ...
    if (bit_action != 'WAIT'):
        ...
        dead_ends = [(x, y) for x in range(1, 16) for y in range(1, 16) if (arena[x, y] == 0)
                     and ([arena[x + 1, y], arena[x - 1, y], arena[x, y + 1], arena[x, y - 1]].count(0) == 1)]
        crates = [(x, y) for x in range(1, 16) for y in range(1, 16) if (arena[x, y] == 1)]
        targets = dead_ends + crates
        bit_action = sel_action_target(self, targets)
        if (bit_action != 'WAIT'):
            idx_bit = self.map_actions[bit_action]
            state[5 + idx_bit] = 1

    # bit_action = sel_action_target(self, others)
    # self.flag_others_in_target = 0
    ...
    state[5:9] = np.logical_and(state[:4], state[5:9])

    #Scape state
    state[9:13] = scape_map(self, x, y, arena, bomb_xys, others, bomb_map)
    #Scape state others
    state[13:25] = scape_map_others(self, arena, bomb_xys, others, bomb_map)

    #Number of crates:
    number_crates = num_crates(self, x, y, arena)
    if  number_crates < 10:
...
def get_reward(self):
    ...
    if (e.COIN_FOUND in self.events) and ( not e.KILLED_SELF in self.events):
    ...
    if e.KILLED_OPPONENT in self.events:
         self.actions_bomb_dropped += 1
         reward += self.reward_list['OPPONENT_ELIMINATED']
         self.logger.debug("OPPONENT_ELIMINATED")

    if e.GOT_KILLED in self.events:
         self.actions_bomb_dropped += 1
         reward += self.reward_list['DIE']
         self.logger.debug("GOT_KILLED")

    if e.SURVIVED_ROUND in self.events:
         self.actions_bomb_dropped += 1
         reward += self.reward_list['COIN_COLLECTED']
         self.logger.debug("SURVIVED_ROUND")
...
def setup(self):
    ...
    self.persistence_update_frequency = 10
